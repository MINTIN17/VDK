...
void setup() {
  ...
  server.on("/", [] {
    server.send(200, "text/html",
                ...
                "var DirectionOfRotationInput = document.getElementById('DirectionOfRotation');"
                "DirectionOfRotationInput.value = opt;"
                "var xhttp2 =  new XMLHttpRequest();"
                "var DirectionOfRotation = DirectionOfRotationInput.value;"
                ...
                "<br><br>"
                "<label>Direction Of Rotation (Cung , Nguoc) : </label>"
                "<input type='text' name='DirectionOfRotation' id = 'DirectionOfRotation'>"
                "<input type='button' value='Send' onclick = sendValue()>"

                "<br>"
                "<br>"
                "<button style='margin-right:30px' onclick = quaydongco('Cung') >Cung Chieu</button>"
                "<button style='margin-right:30px' onclick = quaydongco('Nguoc') >Nguoc Chieu</button>"
...
void handleSendValue() {
  if (server.hasArg("DirectionOfRotation")) {
    if(server.arg("DirectionOfRotation") == "Cung") receivedValue = 1; // nhận số 
    if(server.arg("DirectionOfRotation") == "Nguoc") receivedValue = 0; // nhận số 
  }
}
